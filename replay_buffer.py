...
class ReplayBuffer:
    def __init__(self, args, captain= False):
        self.N = args.N
        ...
        self.episode_num = 0
        self.current_size = 0
        self.buffer = {'obs_n': np.zeros([self.buffer_size, self.episode_limit + 1, self.N, self.obs_dim], dtype=Dtype),
                       's': np.zeros([self.buffer_size, self.episode_limit + 1, self.state_dim], dtype=Dtype),
                       'avail_a_n': np.ones([self.buffer_size, self.episode_limit + 1, self.N, self.action_dim], dtype=Dtype),  # Note: We use 'np.ones' to initialize 'avail_a_n'
                       'last_onehot_a_n': np.zeros([self.buffer_size, self.episode_limit + 1, self.N, self.action_dim], dtype=Dtype),
                       'a_n': np.zeros([self.buffer_size, self.episode_limit, self.N], dtype=Dtype),
                       'r': np.zeros([self.buffer_size, self.episode_limit, 1], dtype=Dtype),
                       'dw': np.ones([self.buffer_size, self.episode_limit, 1], dtype=Dtype),  # Note: We use 'np.ones' to initialize 'dw'
                       'active': np.zeros([self.buffer_size, self.episode_limit, 1], dtype=Dtype)
                       }
