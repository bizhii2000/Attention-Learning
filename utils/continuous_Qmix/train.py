...
from model import QMIXNetwork, RNNQNetwork, RNNGaussianPolicy
from utils import soft_update, hard_update
...
class AgentsTrainer(object):
    def __init__(self, num_agents, obs_shape, action_shape, args):
        self.na = num_agents
        self.obs_shape = obs_shape*self.na
        ...
        self.critics = RNNQNetwork(self.obs_shape, action_shape, args.hidden_dim).to(device=self.device)
        self.critics_target = RNNQNetwork(self.obs_shape, action_shape, args.hidden_dim).to(device=self.device)
        self.critics_optim = Adam(self.critics.parameters(), lr=args.critic_lr)
        hard_update(self.critics_target, self.critics)

        self.actors = RNNGaussianPolicy(self.obs_shape, action_shape, args.hidden_dim).to(device=self.device)
        self.actors_target = RNNGaussianPolicy(self.obs_shape, action_shape, args.hidden_dim).to(device=self.device)
        self.actors_optim = Adam(self.actors.parameters(), lr=args.policy_lr)
        hard_update(self.actors_target, self.actors)

        self.qmix_net = QMIXNetwork(num_agents, 16, self.obs_shape * num_agents).to(device=self.device)
        self.qmix_net_target = QMIXNetwork(num_agents, 16, self.obs_shape * num_agents).to(device=self.device)
    ...
    def update_parameters(self, samples, batch_size, updates, train_policy):
        ...
        for i in range(max_episode_len):
            ...
            total_obs_slice = obs_batch[:,i].squeeze().reshape(-1, self.obs_shape)
            ...
            total_obs_next_slice = obs_next_batch[:,i].squeeze().reshape(-1, self.obs_shape)
